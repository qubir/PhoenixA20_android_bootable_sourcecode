...
prepend_title(const char* const* headers) {
    const char* title[] = { "SDK : MID-A20-Android4.2-V1.3rc4",
...
main(int argc, char **argv) {
    ...
    ui->set_led_flicker(ui->LED_NORMAL);
	...
	//<<<<< To suppurt package place on multi-device , not only sdcard
	if (update_package_ex) {
		char absolutePath[256];
		if (updateFromMutliDevice(update_package_ex, absolutePath)) {
			update_package = absolutePath;
		}
	}
	//>>>>>
    ...
    if (status != INSTALL_SUCCESS) {
        ui->SetBackground(RecoveryUI::ERROR); 
        ui->set_led_flicker(ui->LED_ERROR);
    ...
    if (status != INSTALL_SUCCESS || ui->IsTextVisible()) {
        ...
        ui->Print("\nInstall Failed!");
        for(int i=0;i < 5;i++){
            ui->Print("\n%d second to reboot your system..." , i);
            sleep(1);
        }
        goto reboot;
